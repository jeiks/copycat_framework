...
class Oracle(object):
    def __init__(self, problem, save_filename=None, resume_filename=None,
                 dataset_root='', db_name_train='train', db_name_test='test',
        ...
        if dont_load_datasets:
            ...
            self.dataset = Dataset(problem=self.problem, normalize=True, root=dataset_root, config_fn=self.config_fn)
        ...
        self.model = Model(self.outputs, pretrained=True, oracle_arch=True, state_dict=resume_filename, save_filename=save_filename)
    
    def __load_config(self):
        default = {'max_epochs':10, 'batch_size':16, 'lr':1e-4, 'gamma':0.1, 'criterion':'CrossEntropyLoss',
                   'optimizer':'SGD', 'weight_decay': True, 'validation_step':0, 'save_snapshot':False}
        self.config = Config(self.config_fn).get_problem_options(self.problem, 'oracle')
    ...
    def __check_dataset(self, dataset):
        assert dataset.has_transformer(transforms.Normalize), \
